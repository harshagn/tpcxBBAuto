...
def configurationArg(s):
    ...
    if len(pConfig) == 2 or len(pConfig) == 3:
        ...
        raise argparse.ArgumentTypeError(f"{s} -Invalid Format for configuration option.")

parser = argparse.ArgumentParser("Script to modify configuration for yarn role config group")
...
parser.add_argument('--protocol', nargs='?', help=f"[optional] Protocol to use, Default: {API_PROTOCOL}", default = API_PROTOCOL)
parser.add_argument('--port', nargs='?', help=f"[optional] Port for CM API server, Default: {PORT}", default = PORT)
parser.add_argument('--host', nargs='?', help=f"[optional] Host IP for CM API server, Default: {API_HOST}", default = API_HOST)
parser.add_argument('--apiver', nargs='?', help=f"[optional] API version to use, Default: {API_VERSION}", default = API_VERSION)
parser.add_argument('-c', '--cluster', nargs='?', help=f"[optional] Cluster Name, Default: {CLUSTER}", default = CLUSTER)
parser.add_argument('-u', '--user', nargs='?', help=f"[optional] CM Client User Name, Default: {CM_CLIENT_USERNAME}", default = CM_CLIENT_USERNAME)
parser.add_argument('-p', '--password', nargs='?', help=f"[optional] CM Client User Password, Default: {CM_CLIENT_PASSWORD}", default = CM_CLIENT_PASSWORD)
    ...
    if configuration in config_by_group:
        ...
        if _.index_of(structures_for_config, picked_role_config_group) != -1:
            ...
            try:
                ...
                print(f"Updated: Configuration {configuration}' for '{picked_role_config_group}' config group")
            except ApiException as exc:
                if (args.verbose):
                    print(exc)
                print('Failed to update')
        else:
            print(f"{specified_role_config_group}: The specified role group is not application for the configuration")
    else:
         print(f"{configuration}: No such configuration exists")
